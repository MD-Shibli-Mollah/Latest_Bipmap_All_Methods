...
class Bitmap:
    ...
    def convert_to_grayscale_average_method_2(self):
        for r in range(0, self.height):
            for c in range(0, self.width):
                red, green, blue = self.get_pixel(r, c)
                gray = int(red * 0.21 + green * 0.72 + blue * 0.07)
                self.set_pixel(r, c, gray, gray, gray)

    def convert_to_grayscale_average_method_3(self):
        for r in range(0, self.height):
            for c in range(0, self.width):
                red, green, blue = self.get_pixel(r, c)
    ...
    def extract_key_channel(self):
        for r in range(0, self.height):
            for c in range(0, self.width):
                ...
                # average = int((red + red + red) / 3)
                key = int(K * 255)
                self.set_pixel(r, c, key, key, key)
    ...
    def rotate_clockwise_90(self):
        ...
        for r in range(0, self.height):
            for c in range(0, self.width):
                index = int(self.data_offset + (r * self.width + c) * 3)
                c1 = r
                r1 = int(self.width - 1 - c)
                index2 = int(self.data_offset + (r1 * self.width + c1) * 3)
        for i in range(0, 3):
            self.byte_array[index2 + 1] = bytearray[index + i]
...
bitmap = Bitmap()
bitmap.read_file("s.bmp")
print("The data offset is", bitmap.data_offset)
print("The data height is", bitmap.height)
